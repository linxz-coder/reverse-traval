...
- 全球城市推荐中文旅游区域，先显示酒店结果，再后台规范化片区名称
...
每日推荐使用“高级酒店 + 有泳池 + 有儿童设施”的缓存结果。半夜预热任务默认 02:10 后运行，只预热最近一个法定假期；如果错过凌晨任务，会在 06:00-08:00 之间补跑一次，但当天已经跑过就跳过。预热优先广东主要城市，并每天随机加入 3 个国际热点旅游城市，日志位置：
...
如果配置 `GEONAMES_USERNAME`，后台区域规范化会在酒店坐标可用时尝试调用 GeoNames；未配置时只使用本地中文片区规则，不影响主搜索耗时。
